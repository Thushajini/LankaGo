...
//           alt={places.name}
//           className="w-full h-48 sm:h-75 object-cover hover:scale-100 transition duration-500 "
//         />
...
//           <h1 className="text-2xl sm:text-4xl font-bold">{places.name}</h1>
...
//         </div>
//         <div className="fixed bottom-0 left-0 right-0 bg-[#F8F8F8] px-5 py-4 border-t border-gray-100 grid grid-cols-2 gap-4 ">
//           <button
...
//           <button
//             onClick={() => openMap(places.latitude, places.longitude)}
    ...
    <div className="min-h-screen bg-[#F8F8F8] flex flex-col">
      ...
      <div className="relative">
        <img
          src={imageMap[places.image]}
          alt={places.name}
          className="w-full h-48 sm:h-75 object-cover transition duration-500"
        />
        <button
          onClick={() => navigate(-1)}
          className="absolute top-5 left-5 text-black text-2xl bg-white rounded-full p-2 shadow transition duration-300 hover:scale-125"
        >
          ←
        </button>
        <button
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className="bg-green-50 rounded-2xl p-5 mt-6 flex justify-between items-center shadow-sm hover:shadow-lg transition duration-300">
          <div>
            <h2 className="font-bold text-2xl text-gray-800">
              You are {distance ? `${distance.toFixed(1)}km` : "..."} away
            </h2>
            <p className="text-gray-500 mt-1 text-sm">from this place</p>
          </div>
          <LuSend className="text-green-600 text-3xl" />
        ...
        {/* mt-auto - buttons எப்போதும் content-க்கு நேரடியா கீழே */}
        <div className="grid grid-cols-2 gap-4 mt-auto pt-5">
          <button
            onClick={() => toggleFavorite(Number(places.id))}
            className={`bg-white rounded-xl py-4 font-semibold border transition duration-300 hover:scale-105 ${
              isFavorite
                ? "bg-red-500 text-black border-red-500"
                : "border-red-300"
            }`}
